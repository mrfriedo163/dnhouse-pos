    ...
    <div className="space-y-4">
      ...
      <form className="grid gap-2 sm:grid-cols-5" method="get">
        ...
        <select name="status" defaultValue={searchParams.status ?? ""} className="rounded-lg border border-slate-300 px-3 py-2 text-sm">
          <option value="">Tất cả</option><option value="completed">Đã trả</option>
          <option value="pending">Chưa trả</option><option value="overdue">Quá hạn</option>
        </select>
      ...
      <div className="hidden overflow-x-auto sm:block">
        <table className="w-full text-sm">
          <thead><tr className="text-left text-slate-500">
            <th className="p-2">Mã đơn</th><th>Nhận</th><th>Khách</th><th>SĐT</th><th>Tổng</th><th>Trạng thái</th><th></th>
          </tr></thead>
          <tbody>
            {orders.map((o) => (
      ...
      <div className="space-y-2 sm:hidden">
        {orders.map((o) => (
          <Link key={o.id} href={`/orders/${o.id}`}>
            <Card>
              <div className="flex justify-between"><span className="font-semibold">{o.order_no}</span><span>{o.is_completed ? "✅" : "⏳"}</span></div>
