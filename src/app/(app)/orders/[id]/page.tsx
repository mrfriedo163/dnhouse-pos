    ...
    <div className="space-y-4">
      ...
      <Card>
        <div className="grid gap-2 text-sm sm:grid-cols-2">
          ...
          <div>Trạng thái: {o.is_completed ? "✅ Đã trả" : "⏳ Chưa trả"}</div>
        </div>
      </Card>
      ...
      <Card>
        ...
        <table className="mt-2 w-full text-sm">
          <thead><tr className="text-left text-slate-500"><th>Dịch vụ</th><th>SL</th><th>Đơn giá</th><th className="text-right">Thành tiền</th></tr></thead>
          <tbody>
            {(items ?? []).map((it: any) => (
              <tr key={it.id} className="border-t">
                <td>{it.service_name_snapshot}</td><td>{it.quantity} {it.unit_type}</td>
                <td>{formatVnd(it.unit_price)}</td><td className="text-right">{formatVnd(it.line_total)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <PrintButtons billUrl={o.bill_drive_web_url} orderId={o.id} />
      {profile?.role === "admin" && <AdminOrderActions orderId={o.id} isCompleted={o.is_completed} />}
    </div>
  );
}
