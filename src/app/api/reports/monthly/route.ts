...
import { buildMonthlyData } from "@/lib/report-service";
import { buildMonthlyReport } from "@/lib/excel/report";
...
export async function POST(request: Request) {
  ...
  const { month, upload } = await request.json();
  ...
  const data = await buildMonthlyData(month, shop.shop_name);
  const buf = await buildMonthlyReport(data);
  const fileName = `MonthlyReport-${month}.xlsx`;

  if (upload) {
    try {
      const { drive, settings } = await getConnectedDrive();
      const folderId = await ensureDatedFolder(drive, settings.root_folder_id!, "Monthly Reports", new Date(`${month}-01T00:00:00+07:00`));
      ...
      await admin.from("generated_files").insert({
        file_type: "monthly_report", file_name: up.name, drive_file_id: up.fileId, drive_web_url: up.webViewLink, generated_by: profile.id,
      });
